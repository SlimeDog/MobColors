...
public interface ComplexMobTypeVariant {
    ...
    public static class InstanceTracker<T, V> {
        ...
        protected InstanceTracker(Function<T, V> constructor) {
            this.constructor = constructor;
        }

        protected V get(T variant) {
            return variantsMap.computeIfAbsent(variant, v -> constructor.apply(v));
        }

        protected static <T1, T2, V> InstanceTracker<T1, InstanceTracker<T2, V>> getBiTracker(
                BiFunction<T1, T2, V> constructor) {
            return new InstanceTracker<>(t1 -> new InstanceTracker<T2, V>(t2 -> constructor.apply(t1, t2)));
        }

        protected static <T1, T2, T3, V> InstanceTracker<T1, InstanceTracker<T2, InstanceTracker<T3, V>>> getTripleTracker(
